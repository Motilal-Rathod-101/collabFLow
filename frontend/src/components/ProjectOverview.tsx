...
export default function ProjectOverview({ openDialog }: Props) {
  ...
  const { currentWorkspace, loading, workspaces } = useSelector(
  ...
  useEffect(() => {
    if (isAuthenticated && workspaces.length === 0 && !loading) {
      dispatch(fetchWorkspaces());
    }
  }, [isAuthenticated, workspaces.length, loading, dispatch]);

  useEffect(() => {
    if (isAuthenticated && currentWorkspace && projects.length === 0) {
      dispatch(fetchProjects());
    }
  }, [isAuthenticated, currentWorkspace, projects.length, dispatch]);

  if (!currentWorkspace) return null;

  const workspaceProjects = projects.filter(
    (project) => project.workspace === currentWorkspace.id
  );
                  ...
                  <div className="flex items-center gap-2 ml-4">
                    ...
                    <div
                      className={`w-2 h-2 rounded-full border-2 ${
                        priorityColors[
                          (project.priority ?? "MEDIUM") as ProjectPriority
                        ]
                      }`}
                    />
                  </div>
