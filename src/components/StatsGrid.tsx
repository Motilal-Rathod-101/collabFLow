...
export default function StatsGrid() {
  ...
  useEffect(() => {
    if (currentWorkspace) {
      ...
      const myTasks = currentWorkspace.projects.reduce(
        (acc, project) =>
          acc + project.tasks.filter(t => t.assignee?.email === currentWorkspace.owner.email).length,
        0
      );
      const overdueIssues = currentWorkspace.projects.reduce(
        (acc, project) =>
          acc + project.tasks.filter(t => t.due_date && new Date(t.due_date) < now).length,
        0
      );

      setStats({ totalProjects, activeProjects, completedProjects, myTasks, overdueIssues });
    }
  }, [currentWorkspace]);
  ...
  const statCards = [
    ...
    { icon: CheckCircle, title: "Completed Projects", value: stats.completedProjects, subtitle: `of ${stats.totalProjects} total`, bgColor: "bg-emerald-500/10", textColor: "text-emerald-500" },
  ...
  return (
    <div className="grid grid-cols-1 lg:grid-cols-4 gap-6 my-9">
      {statCards.map(({ icon: Icon, title, value, subtitle, bgColor, textColor }, i) => (
        <div key={i} className="bg-white dark:bg-zinc-950 border border-zinc-200 dark:border-zinc-800 rounded-md hover:border-zinc-300 dark:hover:border-zinc-700 transition duration-200">
          <div className="p-6">
