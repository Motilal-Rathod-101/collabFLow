...
// Define icons and colors for task types
const typeIcons = {
  BUG: { icon: Bug, color: "text-red-500 dark:text-red-400" },
  FEATURE: { icon: Zap, color: "text-blue-500 dark:text-blue-400" },
  TASK: { icon: Square, color: "text-green-500 dark:text-green-400" },
...
const RecentActivity = () => {
  ...
  useEffect(() => {
    ...
    // Normalize task type and status to match our strict types
    const allTasks: Task[] = currentWorkspace.projects.flatMap((project) =>
      project.tasks.map((task: any) => ({
        id: task.id,
        title: task.title,
        type: ["BUG", "FEATURE", "TASK", "IMPROVEMENT", "OTHER"].includes(task.type)
      ...
      {tasks.length === 0 ? (
        ...
        <div className="divide-y divide-zinc-200 dark:divide-zinc-800">
          {tasks.map((task) => {
            ...
            const iconColor = typeIcons[task.type].color;
